...
void *count_algo(void* arg){
	...
	//int thread_id_funct = count_funct->thread_id_copy;
	...
	cout_lock++;
	int thread_id_funct = count_funct->thread_id_copy;
	while(cout_lock < thread_id_funct); //make thread 0 cout the fist
	cout << thread_id_funct << ": " << counts[thread_id_funct] << endl;
	cout_lock++;
